...
async def filter(client, message):
    ...
    if AUTH_CHANNEL:
        ...
        try:
            ...
            await client.send_message(
                chat_id=message.from_user.id,
                text="**කරුණාකර අපේ Updates Channel එකට Join වෙලා ඉන්න!**",
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAACAgUAAxkBAAEDIlthc6AMarUHyIjgF-RGQEXsYGRKdAACzwMAAo_wmVdbn9-67S1JIiEE')
            return
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>ඔයා හොයපු එකෙන් Results කිහිපයක් හමුවුනා 👇 {search} ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))

            else:
                await message.reply_text(f"<b>ඔයා හොයපු එකෙන් Results කිහිපයක් හමුවුනා 👇 {search} ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
            return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        await query.answer("මේක ඔයාට නෙමෙයි මචං. අලුතින් Search කරන්න ❤️",show_alert=True)
